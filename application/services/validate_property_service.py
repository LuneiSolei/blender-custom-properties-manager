...
class ValidatePropertyService:
    ...
    def validate(self, data_path: str, property_name: str, operator) -> bool:
        """
        ...
        operator.data_object = utils.resolve_data_object(bpy.context, data_path)
        if not operator.data_object:
            operator.report({'ERROR'}, f"Data object for '{data_path}' not found")

            return False

        if property_name not in operator.data_object:
            operator.report({'ERROR'}, f"Property '{property_name}' not found in {operator.data_object.name}")
